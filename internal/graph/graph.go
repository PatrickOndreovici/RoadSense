...
func (g *Graph) Dijkstra(startID, endID int64, maxDistance float64, userGraph *UserGraph) (float64, []int64) {
	...
	for pq.Len() > 0 {
		...
		for _, edge := range currentNode.Edges {
			...
			newCost := item.Cost + edge.Distance
			if newCost > maxDistance {
				continue
			}

			if old, ok := dist[nextID]; !ok || newCost < old {
				dist[nextID] = newCost
